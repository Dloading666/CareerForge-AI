...
本仓库**没有单元测试、没有 Python linter**。每次改动后的最低验证标准：

1. 前端：`cd frontend && npm run build && npm run lint` 必须全绿。
2. 后端迁移：`cd backend && alembic heads`（必须只有一个 head）+ 在干净 SQLite 库上 `alembic upgrade head`。
3. 后端逻辑：启动 uvicorn，用 `/docs` 或 curl 打一遍你改过的端点。
4. 涉及对话/工具链的改动：手动走一遍核心剧本——选择工作简历 → 发消息 → AI 调工具 → 前端 activity 胶囊正常渲染 → 简历中心确认结果。
...
4. **迁移命名**：`YYYYMMDD_NNNN_slug`（当前最新 `20260612_0025`）。改动迁移判定链时同步检查 `backend/entrypoint.sh`（容器内对「有表无 alembic_version」自动 stamp 再 upgrade）。
...
| `student/` | **核心**。`agent_runtime.py`（Agentic Loop，5000+ 行）、`router.py`（会话/消息/记忆/提案）、`run_manager.py`（后台 run + SSE）、`resume_router.py`（简历 CRUD/快照/撤销）、`agent_models.py` / `revision_models.py` / `proposal_models.py` |
...
- 学生端页面：`/student` AI简历助手（`AgentChatView agentType="resume"`）· `/student/interviewer`（`AIInterviewerPage`，独立实现）· `/student/resumes[/:id]` 简历中心/编辑器 · 个人中心是 Modal（不是路由）。
