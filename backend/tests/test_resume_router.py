...
class ResumeRouterTests(unittest.TestCase):
    ...
    def test_import_file_network_error_returns_envelope(self):
        """When the LLM provider is unreachable, return 502 with the standard envelope
        instead of leaking a raw 500 plain-text response.

        Regression test for the bug where uncaught httpx.ConnectError /
        TimeoutException / RemoteProtocolError caused a 500 + plain text, breaking
        the frontend apiRequest envelope parsing.
        """
        from app.student import resume_router as rr
        from app.student import resume_import_service as ris

        file_bytes = b"%PDF-1.4\nfake content for unit test" * 20

        def _fake_extract(_content, _name, _ct):
            return "x" * 500  # > _SCANNER_THRESHOLD (200)

        cases = [
            (httpx.ConnectError("connection refused"), 502),
            (httpx.ConnectTimeout("connect timeout"), 502),
            (httpx.ReadTimeout("read timeout"), 502),
            (httpx.RemoteProtocolError("protocol error"), 502),
        ]
        for raised_exc, expected_status in cases:
            with mock.patch.object(ris, "extract_resume_file", _fake_extract), \
                 mock.patch.object(
                     ris, "parse_resume_text_to_data",
            ...
            self.assertEqual(response.status_code, expected_status, msg=str(raised_exc))
            # The response must be a parseable JSON envelope/dict (not a plain
            # text "Internal Server Error" body, which is what the original bug
            # produced and what the frontend apiRequest rejects as
            # "Non-JSON API response").
            self.assertIn("application/json", response.headers.get("content-type", ""))
            body = response.json()
            # Accept either the standard {code, msg, data} envelope or FastAPI
            # HTTPException default {detail: "..."} shape — both convey a usable
            # error to the user. The contract we are enforcing is: valid JSON,
            # non-empty human-readable message, no leaked traceback.
            message = body.get("msg") or body.get("detail")
            self.assertTrue(message, msg=f"empty error message in body={body!r}")

    def test_import_file_unexpected_error_returns_envelope(self):
        """Any unexpected exception in the LLM call path must also return the standard
        envelope (here 500) instead of a plain-text response.
        """
        from app.student import resume_router as rr
        from app.student import resume_import_service as ris
