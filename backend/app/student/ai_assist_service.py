...
_SECTION_LABELS = {
    "experience": "工作经历条目（岗位描述 / 工作内容与成果）",
...
_INSTRUCTION_PROMPTS = {
    "polish": "请润色并提升表达质量，使其更适合中文简历场景；保留事实，不要新增虚假信息。",
    "quantify": "请在保留原意的基础上加入可量化的数据占位（如 \u201c提升 X%\u201d、\u201c服务 X 万用户\u201d），如果原文中没有可量化信息请保持原文。",
    "concise": "请在不丢失关键信息的前提下精简表达，单行不超过 30 个汉字。",
    "expand": "请适度展开表达，补充同类工作场景中常见的关键动作或结果（不要编造公司/项目专有名词）。",
    "translate_en": "请把内容翻译为简洁的英文简历表达，保留专有名词（如公司名、产品名）。",
}

_DEFAULT_INSTRUCTION = (
    "请按用户给定的指令改写；保留事实，不得编造公司/学校/产品名称/数据；输出使用中文（除非指定翻译）。"
)
...
def _build_system_prompt(section: str, instruction_key: str, has_jd: bool) -> str:
    label = _SECTION_LABELS.get(section, "简历文本")
    specific = _INSTRUCTION_PROMPTS.get(instruction_key, _DEFAULT_INSTRUCTION)
    jd_clause = (
        "请同时参考用户提供的目标职位描述（JD），尽量贴近该 JD 的关键词与能力要求。"
        if has_jd
        else "用户没有提供 JD，请按通用简历标准改写。"
    )
    return (
        "You are a resume writing assistant. "
        "You receive the candidate's original text and must return a single strict JSON object (no prose, no markdown fences). "
        'Schema: {"suggested": "<the rewritten text>"}. '
...
def ai_assist_field(
    ...
    *,
    ...
    Returns: {"suggested": str, "model": str, "instruction": str}
    Raises ValueError on failure.
    """
    models = _list_text_models(db)
    if not models:
        raise ValueError(
            "No text model available for students. Please contact admin to enable one in the model plaza."
        )

    system_prompt = _build_system_prompt(section, instruction_key, bool(jd_text and jd_text.strip()))
